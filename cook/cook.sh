...
yes | cp -R prepare/WF-GO/* engine/
yes | cp -R prepare/WF-GO/WFGOcommitHistory.txt engine/
...
yes | cp -R prepare/v6engine-deps/* engine/src/

echo "Moving dependancies files completed."
echo ""
echo "Starting to move v6engine files"

yes | cp -R prepare/v6engine/* engine/src/duov6.com/
